...
`QaLogger` est une **façade** au-dessus de SLF4J (qui reste le moteur d'écriture via Logback). **Deux**
besoins — **non couvrables par la config** — la justifient :

1. **Accumulation de la trace par test** (buffer `ThreadLocal`) : la matière première des artefacts
   `FAIL_*.log` / `ERROR_*.log` produits par `TestFailureManager` (un logger standard ne sait pas
   « rejouer » l'historique des steps d'un test).
2. **Masquage des secrets** avant toute écriture (roadmap étape 7, OWASP) : impossible à imposer si
   chaque projet appelle SLF4J directement — il faut un **point d'interception**.

> L'**uniformité du format / de la console** n'est PAS une raison de `QaLogger` : elle est assurée par
> la **config Logback partagée** (`LogbackConfigurator`, D16-bis). Sans les deux besoins ci-dessus,
> `QaLogger` n'aurait pas de raison d'être (on utiliserait SLF4J directement).
...
| `QaLogger` | **sans état propre**, opère sur le `ThreadLocal` | état dans le ThreadLocal |
| `TestFailureManager` | **sans état**, lit le ThreadLocal + écrit un **dossier unique** `KO__` | chemins uniques → 2 échecs simultanés ne collisionnent pas |
| `LogbackConfigurator` | **one-time** au démarrage JVM (ServiceLoader) | init unique |

### État « par test » + règles de thread-safety

- **Buffer de `QaLogger` = `ThreadLocal`** + **`remove()` obligatoire en fin de test**, porté par le
  listener de D16 (quasi imposé par l'activation native).
